...
def extract_data_full_refresh() -> pl.LazyFrame:
    df = pl.scan_csv("data/employees.csv", infer_schema_length=0)
...
def extract_data_incremental(execution_date: datetime = datetime.now()) -> pl.LazyFrame:
    ...
    df = pl.scan_csv("data/employees.csv", infer_schema_length=0).filter(
        pl.col("join_date") == current_date_str
    )
